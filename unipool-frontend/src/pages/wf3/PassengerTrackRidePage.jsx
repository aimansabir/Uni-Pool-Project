...
export default function PassengerTrackRidePage() {
  ...
  const fetchData = async (isInitial = false) => {
    try {
      ...
      const liveEta = trackRes.data?.estimatedArrivalMinutes;
      if (liveEta != null) {
        setEtaSeconds(Math.round(liveEta * 60));
      } else if (rideRes.data?.durationMin != null) {
        // Fallback: use ride's total estimated duration as rough estimate
        setEtaSeconds(Math.round(rideRes.data.durationMin * 60));
      } else {
        setEtaSeconds(null);
      }
